...
--
-- Temporary table structure for view `vw_ponto`
--

DROP TABLE IF EXISTS `vw_ponto`;
/*!50001 DROP VIEW IF EXISTS `vw_ponto`*/;
SET @saved_cs_client     = @@character_set_client;
SET character_set_client = utf8;
/*!50001 CREATE VIEW `vw_ponto` AS SELECT 
 1 AS `fkpessoa`,
 1 AS `data_inicial`,
 1 AS `data_final`,
 1 AS `horario_inicial`,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_ponto` AS select `ponto`.`fkpessoa` AS `fkpessoa`,`ponto`.`data_inicial` AS `data_inicial`,`ponto`.`data_final` AS `data_final`,`ponto`.`horario_inicial` AS `horario_inicial`,`ponto`.`horario_final` AS `horario_final`,(case when (`horario_padrao`.`possui_horario_almoco` = 1) then `horario_padrao`.`horario_almoco` when (`horario_padrao`.`possui_horario_almoco` = 0) then '00:00:00' end) AS `almoco`,timediff(timediff(`ponto`.`horario_final`,`ponto`.`horario_inicial`),`horario_padrao`.`horario_almoco`) AS `Total` from ((`ponto` left join `pessoa` on((`pessoa`.`id` = `ponto`.`fkpessoa`))) left join `horario_padrao` on((`horario_padrao`.`fkpessoa` = `pessoa`.`id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
